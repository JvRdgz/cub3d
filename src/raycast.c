...
/*   Updated: 2021/04/20 21:04:25 by jarodrig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			initialize_window(t_raycaster *raycaster, t_data *data, t_player *player, t_color *color, t_img *img)
{
	...
	{
		raycasting_control(player, raycaster, data, color);
		raycaster->x++;
...
*/
	...
	raycaster->camera_x = 2 * raycaster->x / (double)raycaster->w - 1;
...
void			raycasting_control(t_player *player, t_raycaster *raycaster, t_data *data, t_color *color)
{
	...
	dda_algorithm(player, raycaster, color);
	set_wall_dimensions(raycaster, player, color);
}
